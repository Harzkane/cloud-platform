...
// Creates:     <appSlug>.nexgenhost.ng → Oracle VM IP
export async function createSubdomain(appSlug: string): Promise<{
  recordId: string;
  subdomain: string;
  url: string;
}> {
  const baseDomain = process.env.BASE_DOMAIN ?? "nexgenhost.ng";
